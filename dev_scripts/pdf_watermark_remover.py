...
def CheckingFiles(dirName):
    ...
    input(
        f"Please place the PDF files in the '{dirName}' Folder Created! and Press Enter..."
    )
    print("-" * 50)
    ...
    while True:
        ...
        if len(inputFilesNames) == 0:
            input(
                f"No PDF files Found in '{dirName}' Folder! Please add and Press Enter..."
            )
        else:
            break
...
def processingFilesInFolder(
    inputFileDir, outputFileDir, inputFilesNames, waterMarkTextStarting
):
...
def deletingFiles(inputDirName, outputDirName):
    ...
    input(
        f"Please Cut PDF files found in '{outputDirName}' and Paste in other folder and Press Enter..."
    )
    print("-" * 50)
    while True:
        ...
        if len(outputDir_files) == 0:
            ...
            input(
                f"'{outputDirName}' is not empty. Please empty folder and Press Enter..."
            )
...
def main():
    ...
    processingFilesInFolder(
        inputDirName, outputDirName, inputFilesNames, waterMarkTextStarting
    )
